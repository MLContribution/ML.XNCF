var app = new Vue({
  ...
  data() {
    var validateCode = (rule, value, callback) => {
      if (this.dialog.data.DocumentType === 3) {
        if (!value) {
    ...
    return {
      ...
      dialog: {
        ...
        visible: false,
        data: {
          id: '', name: '', parentId: [], url: '', icon: '', sort: '', visible: true,
          resourceCode: '', isLocked: false, documentType: ''
        },
        rules: {
          name: [
            { required: true, message: "目录名称为必填项", trigger: "blur" }
          ],
          documentType: [{ required: true, message: "类型为必选项", trigger: "blur" }],
  ...
  watch: {
    'dialog.visible': function (val, old) {
      ...
      if (!val) {
        this.dialog.data = {
          id: '', name: '', parentId: [], url: '', icon: '', sort: '', visible: false,
          resourceCode: '', isLocked: false, DocumentType: ''
        };
  ...
  methods: {
    ...
    // 获取所有目录
    async getList() {
      console.log('开始请求Document');
      const a = await service.get('/Admin/Document/Edit?handler=Document');
      const b = a.data.data;
      let allDocument = [];
      this.ddd(b, null, allDocument);
      this.tableData = allDocument;
    },
    // 数据处理
    ...
    handleEdit(index, row, flag) {
      ...
      // 编辑
      let { id, name, parentId, url, icon, sort, visible,
        resourceCode, isLocked, DocumentType } = row;
      this.dialog.data = {
        id, name, parentId: [parentId], url, icon, sort, visible,
        resourceCode, isLocked, DocumentType
      };
      // dialog中父级目录 做递归显示
      let x = [];
      ...
      if (flag === 'edit') {
        this.dialog.title = '编辑目录';
        if (row.isLocked) {
          this.dialog.disabled = true;
        }
      } else if (flag === 'addNext') {
        this.dialog.data.id = '';
        this.dialog.title = '增加下一级目录';
      ...
      this.$refs['dataForm'].validate(valid => {
        ...
        if (valid) {
          ...
          let data = {
            ...
            DocumentType: this.dialog.data.DocumentType
          };
          service.post("/Admin/Document/Edit", data).then(res => {
            if (res.data.success) {
              this.getList();
              this.$notify({
                title: "Success",
    ...
    handleDelete(index, row) {
      let ids = [row.id];
      service.post("/Admin/Document/edit?handler=Delete", ids).then(res => {
        if (res.data.success) {
          this.getList();
          this.$notify({
            title: "Success",
